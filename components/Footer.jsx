import { assets } from '@/assets/assets'
import Image from 'next/image'
import React from 'react'
import { motion } from "motion/react"

const Footer = (isDarkMode) => {
    return (
        <div className='mt-20'>
            <div className='text-center'>
                <Image src={isDarkMode?assets.logo_dark:assets.logo} alt='logo' className='w-36 mx-auto mb-2' />
                <div className='flex items-center w-max gap-2 mx-auto'>
                    <Image src={ isDarkMode?assets.mail_icon_dark:assets.mail_icon} alt='mail-icon' className='w-6' />
                    ruchisinghmech0509@gmail.com
                </div>
            </div>
            <div className='text-center sm:flex items-center justify-between border-t border-gray-400 mx-[10%] mt-12 py-6'>
                <p> ©2025 Ruchi Singh. All rights reserved. </p>
                <ul className='flex item-center gap-10 justify-center mt-4 sm:mt-0'>
                    <li><a target='_blank' href="https://github.com/ruchi-singh0509">Github</a></li>
                    <li><a href="https://www.linkedin.com/in/ruchi-singh-100956166">Linkedin</a></li>
                </ul>
            </div>
        </div>
    )
}

export default Footer
