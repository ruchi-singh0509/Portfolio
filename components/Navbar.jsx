    ...
    return (
        <>
            <div className='fixed top-0 right-0 w-11/12 -z-10  translate-y-[-80%] dark:hidden'>
                <Image src={assets.header_bg_color} alt='header-backg-color' className='w-full' />
            </div>
            <nav className={`fixed top-0 left-0 right-0 px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 bg-white bg-opacity-50 shadow-sm  ${isScroll ? "bg-white bg-opacity-50 backdrop-blur-lg shadow-sm dark:bg-darkTheme dark:shadow-white/20" : ""}`}>
                <a href="#home">
                ...
                <div className='flex items-center gap-4'>
                    ...
                    <a className='hidden lg:flex items-center gap-3 px-10 py-2.5 border border border-gray-500 rounded-full ml-4 font-Ovo dark:border-white/50' href="#contact">Contact
                ...
                <ul ref={sideMenuRef} className='flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500 dark:bg-darkHover dark:text-white'>
                    <div className='absolute right-6 top-6' onClick={closeMenu}>
                        <Image src={isDarkMode?assets.close_white: assets.close_black} alt='cross-icon' className='w-5' />
                    </div>
